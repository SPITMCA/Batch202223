...
from users.views import medList
...
urlpatterns = [
    ...
    re_path(r'^s/(?P<board>[-\w]+)/(?P<subject>[-\w]+)/$', subjects_views.subject_detail, name='subject_detail'),
    re_path(r'^b/$', boards_views.BoardsPageView.as_view(), name='view_all_boards'),
    re_path(r'^b/(?P<board>[-\w]+)/$', boards_views.BoardPageView.as_view(), name='board'),
    re_path(r'^b/ban_user/(?P<board>[-\w]+)/(?P<user_id>\d+)/$', boards_views.ban_user, name='ban_user'),
    re_path(r'^b/(?P<board>[-\w]+)/edit_board_cover/$', boards_views.edit_board_cover, name='edit_board_cover'),
    re_path(r'^b/(?P<board>[-\w]+)/subscription/$', boards_views.subscribe, name='subscribe'),
    re_path(r'^b/(?P<subject>[-\w]+)/like/$', subjects_views.like_subject, name='like'),
    re_path(r'^load_new_comments/$', comments_views.load_new_comments, name='load_new_comments'),
    re_path(r'^u/(?P<username>[\w.@+-]+)/subscriptions/$',
            boards_views.UserSubscriptionListView.as_view(),
            name='user_subscription_list'),
    re_path(r'^u/(?P<username>[\w.@+-]+)/boards/$',
            boards_views.UserCreatedBoardsPageView.as_view(),
    ...
    re_path(r'^u/follow_user/(?P<user_id>\d+)/$', users_views.follow_user, name='follow_user'),
    ...
    re_path(r'^u/block_spammer/(?P<user_id>\d+)/$', users_views.block_spammer, name='block_spammer'),
    ...
    # login / logout urls
    re_path(r'^login/$', auth_views.LoginView.as_view(extra_context={'form_filling': True}), name='login'),
    ...
    # user registration
    re_path(r'^signup/$', users_views.register_user, name='signup'),

    #med
    path('pleasework/', users_views.pleasework),
    path('med/', medList.as_view()),
    
    #pleasework
    re_path(r'^pleasework/$', users_views.pleasework, name='pleasework'),

    #helpline
    re_path(r'^helpline/$', users_views.helpline, name='helpline'),
    ...
    # profile edit
    re_path(r'^profile_edit/$', users_views.profile_edit, name='profile_edit'),
    re_path(r'^picture_change/$', users_views.change_picture, name='picture_change'),
    ...
    re_path(r'^report_subject/(?P<subject>[-\w]+)/$', reports_views.report_subject, name='report_subject'),
    re_path(r'^report_comment/(?P<pk>\d+)/$', reports_views.report_comment, name='report_comment'),
    re_path(r'^show_reports/(?P<board>[-\w]+)/$', reports_views.show_reports, name='show_reports'),
    re_path(r'^deactivate_subject/(?P<subject>[-\w]+)/$', subjects_views.deactivate_subject,
            name='deactivate_subject'),
    re_path(r'^deactivate_comment/(?P<pk>\d+)/$', comments_views.deactivate_comment, name='deactivate_comment'),
